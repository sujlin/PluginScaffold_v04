...
```text
~/MALightingTechnology/gma3_library/datapools/plugins/{{PLUGIN_SUBDIR}}/{{PLUGIN_NAME}}.xml
~/MALightingTechnology/gma3_library/datapools/plugins/{{PLUGIN_SUBDIR}}/{{PLUGIN_NAME}}.lua
```

同时会生成本地备份：

```text
dist/{{PLUGIN_NAME}}.xml
dist/{{PLUGIN_NAME}}.lua
```
...
```text
Import "{{PLUGIN_SUBDIR}}/{{PLUGIN_NAME}}.xml" At Plugin 1 /NoConfirmation
Call Plugin 1
```
